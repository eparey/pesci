...
def update_ec_optimized_einsum(mat1, mat2, prev_ec, n):
	...
	wcov = np.einsum("ji,ij->j", wmat, cov, optimize='optimal') / sum(prev_ec)
...
def parse_broccoli(input_file, genes_sp_a, genes_sp_b, randomize=False):
	orthologs = []
	paralogs = []
	with open(input_file, 'r') as infile:
		for i, line in enumerate(infile):
			if i == 0:
				continue
			genes = set(line.strip().split('\t')[1].split())
			tmp_genes_a = genes.intersection(genes_sp_a)
			tmp_genes_b = genes.intersection(genes_sp_b)

			if len(tmp_genes_a) == 1 and len(tmp_genes_b) == 1:
				tmp_genes_a, = tmp_genes_a
				tmp_genes_b, = tmp_genes_b
				orthologs.append((tmp_genes_a, tmp_genes_b))

			elif len(tmp_genes_a) > 0 and len(tmp_genes_b) > 0:
				paralogs.append((tuple(tmp_genes_a), tuple(tmp_genes_b)))

	if randomize:
		sp1_homologs = [i[0] for i in paralogs] + [i[0] for i in orthologs]
		sp2_homologs_random = [i[1] for i in paralogs] + [i[1] for i in orthologs]
		random.shuffle(sp2_homologs_random)

		tmp_o, tmp_p = [], []
		for i, g in enumerate(sp1_homologs):
			if isinstance(g, str) and isinstance(sp2_homologs_random[i], str):
				tmp_o.append((g, sp2_homologs_random[i]))
			else:
				if isinstance(g, str):
					g = tuple([g])
				elif isinstance(sp2_homologs_random[i], str):
					sp2_homologs_random[i] = tuple([sp2_homologs_random[i]])

				tmp_p.append((g, sp2_homologs_random[i]))
		orthologs = tmp_o
		paralogs = tmp_p

	# orthologs = random.sample(orthologs, 1000)

	return orthologs, paralogs

def parse_broccoli2(input_file, genes_sp_a, genes_sp_b, randomize=False):
	edges = []
	with open(input_file, 'r') as infile:
		for line in infile:
